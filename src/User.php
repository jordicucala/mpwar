...
class User
{
	...
	public function newUser( $user_data )
	{
		if ( $this->validateData( $user_data ) )
		{
			...
			$user_model = $this->service_provider->getService( 'Development\UserModel' );
			$user_model->addNewUser( $user_data );

			// Send email to user.
			/* @var $mail_class Mail */
			$mail_class = $this->service_provider->getService( 'Development\Mail' );
	...
	private function facebookUser( $user_data )
	{
		...
		$facebook_adapter = $this->service_provider->getService( 'Development\FacebookAdapter' );
